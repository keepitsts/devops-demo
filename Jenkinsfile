...
node {
    ...
    stage('backend tests') {
        try {
            ...
            junit '**/build/**/TEST-*.xml' } } stage('frontend tests') { try {
...
// Testing deliverying artifact to Nexus server
   ...
   stage ('Publish') {
     ...
     nexusPublisher nexusInstanceId: 'stsnexus', nexusRepositoryId: 'maven-releases', packages: [[$class: 'MavenPackage', mavenAssetList: [[classifier: '', extension: '', filePath: '/var/lib/jenkins/jobs/devops-demo-jhipster/lastSuccessful/archive/build/libs/devopsdemo-1.0.0.jar']], mavenCoordinate: [artifactId: 'devops-demo', groupId: 'com.simpletechnologysolutions', packaging: 'jar', version: '1.0.0']]]
   }
}
